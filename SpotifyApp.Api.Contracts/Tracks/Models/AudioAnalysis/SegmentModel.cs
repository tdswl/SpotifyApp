...
public sealed class SegmentModel : IntervalModel
{
    /// <summary>
    ...
    /// with values ranging from 0 to 1 that describe the relative dominance of every pitch in the chromatic scale.
    ...
    /// therefore noisy sounds are likely represented by values that are
    /// all close to 1, while pure tones are described by one value at 1
    /// (the pitch) and others near 0. As can be seen below, the 12
    /// vector indices are a combination of low-power spectrum values
    /// at their respective pitch frequencies. 
    /// </summary>
    [JsonProperty("pitches")]
    public IReadOnlyCollection<double> Pitches { get; set; }
    ...
    /// The timbre feature is a vector that includes 12 unbounded values roughly centered around 0. Those values
    ...
    /// with a stronger attack; etc. See an image below representing the 12 basis functions (i.e. template segments).
    ...
    /// </summary>
    [JsonProperty("timbre")]
    public IReadOnlyCollection<double> Timbre { get; set; }
}